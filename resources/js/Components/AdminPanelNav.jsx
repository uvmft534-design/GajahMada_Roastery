...
export default function AdminPanelNav({ active }) {
  ...
  const navigationNotificationCount = Number(adminNotifications.navigationNotificationCount ?? orderNotificationCount);
  ...
  return <>
    ...
    <div className="sticky top-0 z-40 border-b border-[#2C1E16]/10 bg-[#FDFBF7]/95 backdrop-blur md:hidden">
      <div className="flex h-16 items-center justify-between px-5"><Link href={route('admin.dashboard')} className="flex items-center gap-2.5"><span className="h-8 w-8 overflow-hidden rounded-full border border-[#2C1E16]/15 bg-white"><img src="/images/logo.png" alt="Logo Kopi Gajahmada" className="h-full w-full object-cover"/></span><span><b className="block text-sm">Kopi Gajahmada</b><small className="block text-[9px] font-medium uppercase tracking-[.14em] text-[#2C1E16]/50">Admin</small></span></Link><button type="button" onClick={() => setIsMobileNavOpen((open) => !open)} aria-expanded={isMobileNavOpen} aria-controls="admin-mobile-navigation" aria-label={navigationNotificationCount > 0 ? `Buka navigasi admin, ${navigationNotificationCount} notifikasi perlu ditangani` : 'Buka navigasi admin'} className="relative rounded-full p-2 text-[#2C1E16] transition hover:bg-[#FFE9D2]">{isMobileNavOpen ? <X size={22}/> : <Menu size={22}/>} {navigationNotificationCount > 0 && <span aria-hidden="true" className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-[#D4813E] px-1 text-[9px] font-bold leading-none text-white ring-2 ring-[#FDFBF7]">{navigationNotificationCount > 99 ? '99+' : navigationNotificationCount}</span>}</button></div>
