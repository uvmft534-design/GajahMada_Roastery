              ...
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center justify-between border-t border-[#2C1E16]/5 px-1 pt-3">
                    <div>
                      <span className="text-xl font-bold">{(product.variant_price_from ?? (product.variants?.length === 1 ? product.variants[0]?.price : null)) ? `${product.variants?.length > 1 ? 'Mulai ' : ''}Rp ${Number(product.variant_price_from ?? product.variants[0]?.price).toLocaleString('id-ID')}` : 'Belum tersedia'}</span>
