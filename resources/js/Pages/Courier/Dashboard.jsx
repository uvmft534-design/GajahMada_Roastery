...
export default function CourierDashboard({ orders = [], overview = {} }) {
    ...
    const hasCoordinates = (order) => order.destination_latitude !== null && order.destination_latitude !== ''
        && order.destination_longitude !== null && order.destination_longitude !== ''
        && Number.isFinite(Number(order.destination_latitude)) && Number.isFinite(Number(order.destination_longitude));
    const openMapsUrl = (order) => hasCoordinates(order)
        ? `https://www.google.com/maps/dir/?api=1&destination=${order.destination_latitude},${order.destination_longitude}`
        : `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(order.customer_address)}`;
