...
export default function Checkout({ items, auth, shippingMethods = [], phoneMissing = false }) {
    ...
    const [addressBase, setAddressBase] = useState(auth.user.address ?? '');
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        customer_name: auth.user.name ?? '', customer_phone: auth.user.phone ?? '', customer_address: auth.user.address ?? '', street_name: '', house_number: '', address_detail: '', address_details_applied: false, final_address_preview: '', customer_note: '',
        item_notes: Object.fromEntries(items.map((item) => [item.id, ''])),
        destination_latitude: null, destination_longitude: null,
    }); 
    const subtotal = items.reduce((total, item) => total + Number(item.variant?.price || 0) * item.qty, 0);
    ...
    const updateAddressDetail = (field, value) => {
        setData(field, value);
        setData('address_details_applied', false);
    ...
    const applyAddressDetails = () => {
        ...
        addressParts.push(addressBase || data.customer_address);
        const completeAddress = addressParts.join(', ');
        setData({
            ...data,
            customer_address: completeAddress,
            final_address_preview: completeAddress,
            address_details_applied: true,
        });
    ...
    const switchToManualAddress = () => {
        setAddressSource('manual');
        setAddressBase(data.customer_address);
    ...
    const useProfileAddress = () => {
        setAddressSource('manual');
        setAddressBase(auth.user.address ?? '');
    ...
    const fillAddressFromCurrentLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(async ({ coords }) => {
            try {
                ...
                setAddressBase(response.data.address);
                            ...
                            <div className="mt-2 grid gap-x-8 sm:grid-cols-2">
                                ...
                                <div className="border-t border-[#2C1E16]/10 py-5 sm:col-span-2">
                                    ...
                                    <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                                        <p className="text-sm font-bold text-amber-900">Lengkapi detail alamat</p>
                                        <p className="mt-1 text-xs leading-relaxed text-amber-800/80">Tambahkan nama jalan, nomor rumah, serta detail atau patokan agar kurir lebih mudah menemukan alamat Anda.</p>
                                        <div className="mt-3 grid gap-3 sm:grid-cols-2">
                                            <div><label htmlFor="street_name" className="text-xs font-bold text-amber-900">Nama jalan {needsManualAddressDetails && '*'}</label><input id="street_name" value={data.street_name} onChange={(event) => updateAddressDetail('street_name', event.target.value)} placeholder="Contoh: Gajah Mada" className="mt-1 w-full rounded-xl border border-amber-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#D4813E] focus:ring-0" /><p className="mt-1 text-xs text-red-600">{addressDetailErrors.street_name || errors.street_name}</p></div>
                                            <div><label htmlFor="house_number" className="text-xs font-bold text-amber-900">Nomor rumah {needsManualAddressDetails && '*'}</label><input id="house_number" value={data.house_number} onChange={(event) => updateAddressDetail('house_number', event.target.value)} placeholder="Contoh: 12A" className="mt-1 w-full rounded-xl border border-amber-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#D4813E] focus:ring-0" /><p className="mt-1 text-xs text-red-600">{addressDetailErrors.house_number || errors.house_number}</p></div>
                                        </div>
                                        <div className="mt-3"><label htmlFor="address_detail" className="text-xs font-bold text-amber-900">Detail / Patokan <span className="font-normal">Opsional</span></label><input id="address_detail" value={data.address_detail} onChange={(event) => updateAddressDetail('address_detail', event.target.value)} placeholder="Contoh: Gang SMEA, rumah pagar putih" className="mt-1 w-full rounded-xl border border-amber-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#D4813E] focus:ring-0" /><p className="mt-1 text-xs text-red-600">{errors.address_detail}</p></div>
                                        <button type="button" onClick={applyAddressDetails} className="mt-4 rounded-xl bg-[#D4813E] px-4 py-2 text-sm font-bold text-white transition hover:bg-[#B86632]">Ubah alamat lengkap</button>
                                        {data.final_address_preview && <div className="mt-4 rounded-xl border border-emerald-200 bg-white px-4 py-3 text-sm text-[#2C1E16]"><p className="text-xs font-bold uppercase tracking-[.12em] text-emerald-700">Alamat lengkap yang diisi</p><p className="mt-1 leading-6">{data.final_address_preview}</p></div>}
                                    </div>
                                </div>
                            </div>
                        </div>
