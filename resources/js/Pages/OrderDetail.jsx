                ...
                {items.map((item) => (
                  <div key={item.order_item_id} className="flex items-start justify-between gap-4 border border-[#2C1E16]/10 rounded-2xl p-4">
                    <div className="flex min-w-0 flex-1 items-start gap-3">
                      ...
                      {item.weight_grams && <div className="mt-1 text-xs text-[#2C1E16]/60">Berat: {formatWeightGrams(item.weight_grams)}</div>}
                      <div className="mt-1 text-xs text-[#2C1E16]/60">Metode seduh: {formatOrderItemBrewMethod(item.brew_method, '—')}</div>
